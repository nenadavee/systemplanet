...
Lbol_interp = joblib.load('./Data/Grids/PMS_interp.pkl')
...
class System():
    ...
    def generate(self):
        ...
        for i in range(self.Nplanets):
            ...
            while Per_check == False:
                ...
                # [10, 9, 18.3]
                self.sma[i] = (((self.period[i] * 24 * 60 * 60)**2 * G * self.Mstar * M_sun / (4 * np.pi * np.pi))**(1/3))
        ...
        while Stableorb_check == False:
            ...
            for p in range(pairs):
                ...
                self.hillradius = (((self.sorted_sma[sorted_i] + self.sorted_sma[sorted_k]) * AU)/ 2) * (((self.sorted_Mcore[sorted_i] / self.sorted_Mcore[sorted_k]) * / 3 * self.Mstar) ** (1/3))
                     # go over hill radius units 

                print("Mutual Hill Radius", sorted_i, "-", sorted_k, ":", self.hillradius)
                

                if self.hillradius >= 8.0: 
    ...
    def plot_orbits(self):
         ...
         for i in range(self.Nplanets):
             ...
             orbit = pyasl.KeplerEllipse(sma, ecc, incl, perias)

             # 3D coordinates 
             true_anomaly = np.linspace(0, 2*np.pi, num_points)
             x, y, z = orbit.xyzPos(true_anomaly)
        ...
        #     x = self.sma[i] * (np.cos(theta) * np.cos(self.periastron[i] + theta) - np.sin(theta) * np.sin(self.periastron[i] + theta) * np.cos(self.inclination[i]))
        #     y = self.sma[i] * (np.sin(theta) * np.cos(self.periastron[i] + theta) + np.cos(theta) * np.sin(self.periastron[i] + theta) * np.cos(self.inclination[i]))
        #     z = self.sma[i] * np.sin(self.periastron[i] + theta) * np.sin(self.inclination[i])
            
        #     ax.plot(x, y, z)
            
        # ax.set_xlabel('Orbitx')
        # ax.set_ylabel('Orbity')
        # ax.set_zlabel('Orbitz')
        
        # plt.show() # System.plot_orbits() ? do i need to do this i dont see a System.generate()
    ...
    def probabilities(self):
        ...
        for i in range(self.Nplanets):
            while Bimpact_check == False: # move to probability 
                ...
                if self.bimpactpar[i] < (1 + (self.Rplanet / self.Rstar)):
                    Bimpact_check = True

                    print (i, ":", self.bimpactpar[i])
...
# ##########  IGNORE ALL BELOW, THESE ARE RANDOM PLOTTING ROUTINES I WAS PLAYING WITH ############


# df_planets, df_ratios = run_systems(10000, cmass_disp=3.0, X_disp=0.3)


# df_planets.to_csv('system_planets.csv')
# df_ratios.to_csv('system_ratios.csv')

# df_planets_det = df_planets.query('detected == 1')
# df_ratios_det = df_ratios.query('ratio_detect == 1')

# plt.figure(1, figsize=(5,5))
# plt.scatter(df_planets['period'], df_planets['prad'], s=20, color='grey', alpha=0.05)
# plt.scatter(df_planets_det['period'], df_planets_det['prad'], s=20, edgecolors='black', alpha=0.8, facecolors='none')
# plt.xscale('log')
# plt.yscale('log')
# plt.xlim([1,100])
# plt.ylim([0.8,4.0])
...
# plt.xlabel('Orbital Period (days)')
# plt.ylabel(r'Planet Size (R$_\oplus$)')
# plt.tight_layout()
...
# SESE = df_ratios_det.query('ratio_type == "SE_SE"')
# SESN = df_ratios_det.query('ratio_type == "SE_SN"')
# SNSN = df_ratios_det.query('ratio_type == "SN_SN"')
# SNSE = df_ratios_det.query('ratio_type == "SN_SE"')

# # plt.scatter(df_ratios_det['PjPi'], df_ratios_det['RjRi'], s=20, edgecolors='black', alpha=0.5, facecolors='none', zorder=2)
# plt.scatter(SESE['PjPi'], SESE['RjRi'], color='C0', s=20, alpha=0.3, label='SE SE', zorder=2)
# plt.scatter(SESN['PjPi'], SESN['RjRi'], color='C1', s=20, alpha=0.3, label='SE SN')
# plt.scatter(SNSN['PjPi'], SNSN['RjRi'], color='C2', s=20, alpha=0.3, label='SN SN')
# plt.scatter(SNSE['PjPi'], SNSE['RjRi'], color='C3', s=20, alpha=0.6, label='SN SE')
# plt.xscale('log')
# plt.yscale('log')
# plt.xlim([1,100])
# plt.ylim([0.3,3.1])
# plt.yticks([0.5,1.0,2.0], ['0.5','1.0','2.0'])
# plt.xlabel(r'P$_j$ / P$_i$')
# plt.ylabel(r'R$_j$ / R$_i$')
...
# plt.figure(3)
# Pij_bins = np.logspace(log10(1.0), log10(100), 10)
# Pij_mid = [10**(0.5*log10(Pij_bins[i+1]*Pij_bins[i])) for i in range(len(Pij_bins)-1)]
# # print(Pij_bins)

# SESE_fraction = [0.0] * len(Pij_mid)
# SESN_fraction = [0.0] * len(Pij_mid)
# SNSN_fraction = [0.0] * len(Pij_mid)
# SNSE_fraction = [0.0] * len(Pij_mid)

# SESE_hist, _ = np.histogram(SESE['PjPi'].values, bins = Pij_bins)
# SESN_hist, _ = np.histogram(SESN['PjPi'].values, bins = Pij_bins)
# SNSN_hist, _ = np.histogram(SNSN['PjPi'].values, bins = Pij_bins)
# SNSE_hist, _ = np.histogram(SNSE['PjPi'].values, bins = Pij_bins)

# for i in range(len(Pij_mid)):

#     SESE_fraction[i] = SESE_hist[i] / (SESE_hist[i]+SESN_hist[i]+SNSN_hist[i]+SNSE_hist[i])
#     SESN_fraction[i] = SESN_hist[i] / (SESE_hist[i]+SESN_hist[i]+SNSN_hist[i]+SNSE_hist[i])
#     SNSN_fraction[i] = SNSN_hist[i] / (SESE_hist[i]+SESN_hist[i]+SNSN_hist[i]+SNSE_hist[i])
#     SNSE_fraction[i] = SNSE_hist[i] / (SESE_hist[i]+SESN_hist[i]+SNSN_hist[i]+SNSE_hist[i])
...
# # print(SESE_fraction)
# # print(SNSN_fraction)
# plt.plot(Pij_mid, SESE_fraction, color='C0', label='SE SE')
# plt.plot(Pij_mid, SESN_fraction, color='C1', label='SE SN')
# plt.plot(Pij_mid, SNSN_fraction, color='C2', label='SN SN')
# plt.plot(Pij_mid, SNSE_fraction, color='C3', label='SN SE')
# plt.xscale('log')
# plt.xlim([1,100])
# # plt.yscale('log')
# plt.xlabel(r'P$_j$ / P$_i$')
...
#################### multi observations ###################
# kepler_multis = pd.read_csv('../data/KeplerMultis.txt', delimiter=' ')
# systems = np.unique(kepler_multis['KIC'].values)

# multis = []
# print(len(systems))
# for i in range(len(systems)):
#     _system = kepler_multis.loc[kepler_multis['KIC'] == systems[i]]
#     if len(_system) > 1:
#         multis.append(_system.values)

# # for i in range()

# Pmin = []
# for i in range(len(multis)):
#     Pmin.append(min(multis[i][:,2]))

# logDeltaP = []
# sigma = []
# for i in range(len(multis)):
#     _system = multis[i]
#     _system = _system[_system[:, 2].argsort()]
#     _intrasigma = []
#     for j in range(len(_system)-1):
#         logDeltaP.append(log10(_system[j+1, 2]) - log10(_system[j, 2]))
#         _intrasigma.append(log10(_system[j+1, 2]) - log10(_system[j, 2]))
#     if len(_intrasigma) > 1:
#         sigma.append(np.std(_intrasigma))

# plt.figure(1)
# bins = np.logspace(0,2,20)
# plt.hist(Pmin, bins=bins)
# plt.plot(bins, [70 * exp(-(log10(i)-log10(6))**2 / (2*0.35*0.35)) for i in bins])
# plt.xlabel(r'P$_{min}$ (days)')
# plt.xscale('log')

# plt.figure(2)
# bins = np.linspace(0,2,20)
# plt.hist(logDeltaP)#, bins=bins)
# plt.plot(bins, [600 * (i / 0.3) * exp(-i**2 / (2*0.3*0.3)) for i in bins])
# plt.xlabel(r'log $\Delta P$')
# # plt.xscale('log')

# plt.figure(3)
# plt.hist(sigma, bins=40)#, bins=bins)
# # plt.plot(bins, [600 * 1.0(i / 0.3) * exp(-i**2 / (2*0.3*0.3)) for i in bins])
# plt.xlabel(r'log(Intra system period dispersion)')
# # plt.xscale('log')
...
# df_planets.to_csv('system_planets_randMc.csv')
# df_ratios.to_csv('system_ratios_randMc.csv')
# df = ['system_ratios_constMc.csv', 'system_ratios_randMc.csv']
# linestyle = ['-', '--']
# label = ['const Mc', 'rand Mc']

# for k in range(len(df)):
#     df_ratios = pd.read_csv(df[k])
#     SESE = df_ratios.query('ratio_type == "SE_SE"')
#     SESN = df_ratios.query('ratio_type == "SE_SN"')
#     SNSN = df_ratios.query('ratio_type == "SN_SN"')

#     plt.figure(1)
#     plt.scatter(SESE['PjPi'], SESE['RjRi'], s=0.5, color='red', alpha=0.5, label='SE SE')
#     plt.scatter(SESN['PjPi'], SESN['RjRi'], s=0.5, color='purple', alpha=0.5, label='SE SN')
#     plt.scatter(SNSN['PjPi'], SNSN['RjRi'], s=0.5, color='blue', alpha=0.5, label='SN SN')
#     plt.xscale('log')
#     plt.yscale('log')
#     plt.xlabel(r'P$_j$ / P$_i$')
#     plt.ylabel(r'R$_j$ / R$_i$')
...
#         SESE_fraction[i] = SESE_hist[i] / (SESE_hist[i]+SESN_hist[i]+SNSN_hist[i])
#         SESN_fraction[i] = SESN_hist[i] / (SESE_hist[i]+SESN_hist[i]+SNSN_hist[i])
#         SNSN_fraction[i] = SNSN_hist[i] / (SESE_hist[i]+SESN_hist[i]+SNSN_hist[i])
...
#     # print(SESE_fraction)
#     # print(SNSN_fraction)
#     plt.plot(Pij_mid, SESE_fraction, linestyle[k], color='red', label='SE SE {0}'.format(label[k]))
#     plt.plot(Pij_mid, SESN_fraction, linestyle[k], color='purple', label='SE SN {0}'.format(label[k]))
#     plt.plot(Pij_mid, SNSN_fraction, linestyle[k], color='blue', label='SN SN {0}'.format(label[k]))
#     plt.xscale('log')
#     # plt.yscale('log')
#     plt.xlabel(r'P$_j$ / P$_i$')
#     plt.ylabel(r'Fraction')
